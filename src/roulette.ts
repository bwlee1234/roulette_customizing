...
export class Roulette extends EventTarget {
  ...
  private _winningCount = 1;
  private _goalDist: number = Infinity;
  private _isRunning: boolean = false;
  private _selectedWinners: Marble[] = [];
  ...
  private _update() {
    ...
    if (this._stage) {
      this._camera.update({
        ...
        targetIndex: this._getTargetActiveIndex(),
      });
    }
  ...
  private _updateMarbles(deltaTime: number) {
    ...
    reachedGoal
      ...
      .forEach((marble) => {
        ...
        if (this._isRunning && this._winners.length >= this._winningCount) {
          this._finishRace();
        }
        setTimeout(() => {
    ...
    const targetIndex = this._getTargetActiveIndex();
  ...
  private _calcTimeScale(): number {
    ...
    const targetIndex = this._getTargetActiveIndex();
    if (this._winners.length < this._winningCount && this._goalDist < zoomThreshold) {
      if (
  ...
  private _render() {
    ...
    const renderParams = {
      ...
      winningCount: this._winningCount,
      selectedWinners: this._selectedWinners,
  ...
  public start() {
    ...
    this._selectedWinners = [];
    this._winningCount = this._normalizeWinningCount(options.winningCount);
    this._camera.startFollowingMarbles();

    const activateMarbles = () => {
      this.physics.start();
    ...
    if (this._autoRecording) {
      this._recorder
        .start()
        .then(activateMarbles)
        .catch((error) => {
          console.warn('[Roulette] Recording failed. Starting without recording.', error);
          activateMarbles();
        });
    } else {
      activateMarbles();
  ...
  public setWinningCount(count: number) {
    this._winningCount = this._normalizeWinningCount(count);
  }

  public setWinningRank(rank: number) {
    this.setWinningCount(rank + 1);
  }

  public setAutoRecording(value: boolean) {
    this._autoRecording = value && this._recorder.isSupported;
  }

  public isRecordingSupported() {
  ...
  public setMarbles(names: string[]) {
    ...
    members.forEach((member) => {
      if (member) {
        const availableSlots = maxMarbleCount - totalCount;
        member.count = Math.max(0, Math.min(Math.floor(member.count), availableSlots));
  ...
  public getMaxMarbleCount() {
    return maxMarbleCount;
  }
  ...
  private _getTargetActiveIndex() {
    if (this._winners.length === 0) {
      return 0;
    }
    return Math.max(0, this._winningCount - this._winners.length - 1);
  }

  private _normalizeWinningCount(count: number) {
    const parsedCount = Math.floor(Number(count));
    const safeCount = Number.isFinite(parsedCount) && parsedCount > 0 ? parsedCount : 1;
    return Math.min(safeCount, Math.max(this._marbles.length, 1));
  }

  private _finishRace() {
    if (!this._isRunning) return;

    this._selectedWinners = this._winners.slice(0, this._winningCount);
    this._isRunning = false;
    this._particleManager.shot(this._renderer.width, this._renderer.height);
    this.dispatchEvent(
      new CustomEvent('goal', {
        detail: {
          winners: this._selectedWinners.map((winner, index) => ({
            rank: index + 1,
            name: winner.name,
          })),
        },
