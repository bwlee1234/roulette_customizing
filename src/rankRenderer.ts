...
export class RankRenderer implements UIObject {
      ...
      if (navigator.clipboard) {
        ...
        tsv.push(
          ...[...this.winners, ...this.marbles].map((m) => {
            rank++;
            return [rank.toString(), m.name, rank <= this.winningCount ? '☆' : ''].join('\t');
          })
        );
  ...
  render(
    ...
    { winners, marbles, winningCount, theme }: RenderParameters,
    width: number,
    height: number
  ) {
    ...
    winners.forEach((marble: { hue: number; name: string }, rank: number) => {
      ...
      if (y >= startY && y <= startY + ctx.canvas.height) {
        ...
        const prefix = rank < winningCount ? '☆' : '\u2714';
        ctx.strokeText(`${prefix} ${marble.name} #${rank + 1}`, startX, 20 + y);
        ctx.fillText(`${prefix} ${marble.name} #${rank + 1}`, startX, 20 + y);
      }
    });
    ctx.font = '10pt sans-serif';
    marbles.forEach((marble: { hue: number; name: string }, rank: number) => {
      const totalRank = rank + winners.length;
      const y = totalRank * this.fontHeight;
      if (y >= startY && y <= startY + ctx.canvas.height) {
        ctx.fillStyle = `hsl(${marble.hue} 100% ${theme.marbleLightness}%)`;
        const prefix = totalRank < winningCount ? '☆ ' : '';
        ctx.strokeText(`${prefix}${marble.name} #${totalRank + 1}`, startX, 20 + y);
        ctx.fillText(`${prefix}${marble.name} #${totalRank + 1}`, startX, 20 + y);
      }
    });
    ctx.restore();
  }
